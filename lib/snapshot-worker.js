...
(async () => {
  ...
  try {
    ...
    const snap = await buildLiveSnapshot(current && {
      entities: current.snapshot.entities,
      retrievedAt: current.snapshot.retrievedAt,
      authorityFreshness: current.snapshot.authorityFreshness,
    });
    ...
    writeCache(snap.entities, {
      ...
      isLive: true,
      authorityFreshness: snap.authorityFreshness || {},
    });
