...
function tokenSim(a, b) {
  ...
  if (jw >= 0.55) {
    if (soundex(a) === soundex(b)) s = Math.max(s, 0.9);
    s = Math.max(s, Math.min(0.95, jaroWinkler(foldTranslit(a), foldTranslit(b))));
  }
  return s;
}
...
function applyModifiers(score, record, mods) {
  ...
  if (mods.country) {
    const cc = String(mods.country).toUpperCase().trim();
    const hay = [
      ...(record.addresses || []).map((a) => a.country || a.full || ''),
      ...(record.attributes || []).filter((a) => /nationalit|citizen/i.test(a.label)).map((a) => a.value),
    ].join(' | ').toUpperCase();
    if (hay.trim()) {
      if (cc && hay.includes(cc)) { out = Math.min(1, out + 0.04); corroborated = true; }
      else if (cc) { out -= 0.04; conflict = true; }
    }
  }
