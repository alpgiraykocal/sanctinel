...
function profile(snapshot) {
  ...
  for (const e of entities) {
    ...
    for (const attr of e.attributes || []) {
      attributes++;
      // Derived here rather than read off the record: nothing annotates the
      // snapshot, because writing a property onto every attribute object costs
      // 114MB of resident memory and this runs once per build.
      const kind = attributeKind(attr.label);
      if (kind === 'other') attributesUnclassified++;
      if (kind === 'dob') {
        dobValues++;
        if (parseDateValue(attr.value)) { dobParsed++; hasDob = true; }
      }
    ...
    for (const id of e.identifiers || []) {
      identifiers++;
      if (identifierKind(id.type) === 'other_id') identifiersUnclassified++;
    }
