...
// A date only counts if it exists on the calendar: "31/02/2020" round-trips to
// an invalid Date, and passing it through produced a record that sorted into
// the timeline but rendered with a blank date.
...
function normalizeDate(v) {
  ...
  if (m) return isRealDate(+m[1], +m[2], +m[3]) ? `${m[1]}-${m[2]}-${m[3]}` : '';
  m = s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/); // day/month/year (UK OFSI)
  if (m) {
    const d = Number(m[1]), mo = Number(m[2]), y = Number(m[3]);
    if (isRealDate(y, mo, d)) return `${y}-${String(mo).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
  }
  return '';
}
...
function countriesOf(e) {
  ...
  for (const a of e.attributes || []) {
    /*
     * By canonical kind, not by label. Matching the two literal strings meant
     * OFAC's own wording — `Nationality Country` (5,821 attributes) and
     * `Citizenship Country` (1,142) — never matched, so for OFAC parties this
     * silently fell through to the address country and the chart described
     * where a party is located rather than what it is a national of. Exactly
     * the failure the vocabulary layer exists to end.
     */
    const kind = a.kind || attributeKind(a.label);
    if (kind === 'nationality' || kind === 'citizenship') {
...
function compute(snapshot) {
  ...
  for (const e of entities) {
    ...
    // Counted per MEMBERSHIP, so these sum to more than the entity total: an
    // entity on both the Consolidated List and the CMIC list is on both, and
    // "how many parties carry the CMIC securities restriction" is the question
    // this row is actually asked.
    for (const l of (e.lists && e.lists.length) ? e.lists : [e.list]) bump(byList, l);
    ...
    const countries = countriesOf(e);
    for (const c of countries) bump(byCountry, c);
    ...
    recent.push({
      id: e.id, name: e.name, authority, list: e.list, lists: e.lists || [e.list], type: e.type || 'Unknown',
      ...
      country: countries.values().next().value || '',
    });
  }
