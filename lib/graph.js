...
function egoNetwork(entities, centerId, depth = 2, limits = {}) {
  ...
  for (const e of entities) {
    ...
    for (const rel of e.relationships || []) {
      ...
      edges.push({ source: from, target: to, type: rel.type, ownership: OWNERSHIP_RE.test(rel.type), role });
    }
  }
...
module.exports = { egoNetwork, buildAdjacency, ownershipRole, OWNERSHIP_RE };
