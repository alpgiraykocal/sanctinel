 ...
 *
 * An edge runs from the entity that carries the relationship to the party it
 * names, and OFAC's wording flips the direction between types: "Owned or
 * Controlled By" means the source is the subsidiary, while "Owns, controls, or
 * operates" means the source is the owner. Treating both as one undirected
 * "ownership" bucket is fine for drawing a line, but a hierarchy built on it
 * would invert half the chains — and an inverted 50 Percent Rule chain points
 * the analyst at the wrong party. So resolve the direction explicitly.
 *
 *   'owned_by' → target owns source   (parent = target)
 *   'owns'     → source owns target   (parent = source)
 *   null       → not an ownership edge; control/agency/association only, which
 *                the 50 Percent Rule does not reach.
 */
const OWNED_BY_RE = /owned|property in the interest of|subsidiary of|controlled by/i;
const OWNS_RE = /^owns|owns,|owner of|parent of|controls, or operates/i;

function ownershipRole(type) {
  const t = String(type || '');
  if (OWNS_RE.test(t)) return 'owns';
  if (OWNED_BY_RE.test(t)) return 'owned_by';
  return null;
}

/*
 * Which way an edge stacks in the hierarchy view, for EVERY relationship type
 * and not just the ownership ones.
 *
 * OFAC phrases a relationship from the listed party outward — "Providing
 * support TO", "Acting for or on behalf OF", "Leader or official OF" — so the
 * named party is the principal and belongs above. The exceptions are the types
 * phrased the other way ("Owns, controls, or operates") and the symmetric ones
 * (family, association), which describe no hierarchy at all and sit level.
 *
 *   'up'   → target is the principal/superior; draw it above the source
 *   'down' → target is the subordinate; draw it below the source
 *   'peer' → no direction; same level
 *
 * Unknown types default to 'up', which is what the "X of/to Y" phrasing OFAC
 * uses throughout means. Ownership stays separately identified via
 * ownershipRole/OWNERSHIP_RE: only it drives the 50 Percent Rule, and the view
 * must keep saying so even while it draws the rest.
 */
const PEER_RE = /family|associate|related to|spouse|sibling|relative/i;

function relationDirection(type) {
  const role = ownershipRole(type);
  if (role === 'owns') return 'down';
  if (role === 'owned_by') return 'up';
  if (PEER_RE.test(String(type || ''))) return 'peer';
  return 'up';
}
...
function egoNetwork(entities, centerId, depth = 2, limits = {}) {
  ...
  for (const e of entities) {
    ...
    for (const rel of e.relationships || []) {
      ...
      edges.push({
        source: from, target: to, type: rel.type,
        ownership: OWNERSHIP_RE.test(rel.type), role,
        hier: relationDirection(rel.type),
      });
    }
  }
...
module.exports = { egoNetwork, buildAdjacency, ownershipRole, relationDirection, OWNERSHIP_RE };
