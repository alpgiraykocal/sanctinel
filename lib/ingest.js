...
async function buildAdvancedSnapshot(pub) {
  ...
  return finalizeSnapshot(entities, { source: 'OFAC SLS (advanced /entities)', publicationId: pub.id, publishedDate: pub.date, publications: pub.history, retrievedAt: new Date().toISOString() });
}

// Fallback path: classic flat files (name/alias/address + remarks-derived features).
async function buildFlatSnapshot(pub) {
  ...
  return finalizeSnapshot(entities, { source: 'OFAC SLS (flat files)', publicationId: pub.id, publishedDate: pub.date, publications: pub.history, retrievedAt: new Date().toISOString() });
}

async function buildLiveSnapshot() {
  const pub = await fetchPublicationInfo();
  try {
    return await buildAdvancedSnapshot(pub);
  } catch (e) {
    console.warn(`Advanced snapshot failed (${e.message}). Falling back to flat files.`);
    return await buildFlatSnapshot(pub);
  }
}
...
function finalizeSnapshot(entities, meta) {
  ...
  return Object.freeze({
    ...
    entities, count: entities.length, lists, programs,
  });
}
