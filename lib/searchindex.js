...
/*
 * Same index, built in slices with the event loop free between them.
 *
 * build() is one synchronous pass over every entity — about 1.8s here and
 * several times that on a small instance. Run during boot it blocks the process
 * outright, so incoming requests get no response at all and the platform proxy
 * answers 502: a cold start looks like an outage. Yielding every few thousand
 * entities costs a little throughput and buys the server the ability to say
 * "still loading" while it loads.
 *
 * The result is identical to build() — same passes, same order, only the
 * scheduling differs.
 */
const BUILD_CHUNK = 2000;

async function buildAsync(entities, onProgress) {
  let gram = new Map(), idIdx = new Map(), acr = new Map(), bigram = new Map();
  const push = (map, key, i) => { const a = map.get(key); if (a) a.push(i); else map.set(key, [i]); };

  for (let start = 0; start < entities.length; start += BUILD_CHUNK) {
    const end = Math.min(start + BUILD_CHUNK, entities.length);
    for (let i = start; i < end; i++) indexEntity(entities[i], i, { gram, idIdx, acr, bigram, push });
    if (typeof onProgress === "function") onProgress(end, entities.length);
    await new Promise((r) => setImmediate(r));
  }

  const index = {
    gram: toCsr(gram), idIdx: toCsr(idIdx), acr: toCsr(acr),
    bigram: toCsr(bigram, true),
    n: entities.length,
  };
...
// One entity's postings. Shared by build() and buildAsync() so the two can
// never drift — a difference between them would be a recall difference that
// only shows up on whichever path a given deployment happens to take.
function indexEntity(e, i, ctx) {
  const { gram, idIdx, acr, bigram, push } = ctx;
  const grams = new Set();
  ...
  for (const nm of searchableNames(e)) {
...
function build(entities) {
  ...
  const ctx = { gram, idIdx, acr, bigram, push };
  entities.forEach((e, i) => indexEntity(e, i, ctx));
...
module.exports = { build, buildAsync, candidates };
