...
/*
 * Annotate one record in place: a `kind` on every attribute and identifier, and
 * a parsed interval on the attributes that state a date.
 *
 * In place and at ingest, because both the server and the iOS export read the
 * same finalized snapshot and neither should be re-deriving this per query.
 */
const { parseDateValue } = require('./dates');

function annotateRecord(record) {
  for (const a of record.attributes || []) {
    if (!a.kind) a.kind = attributeKind(a.label);
    if (!a.date && ATTRIBUTE_DATE_KINDS.has(a.kind)) {
      const parsed = parseDateValue(a.value);
      if (parsed) a.date = parsed;
    }
  }
  for (const i of record.identifiers || []) {
    if (!i.kind) i.kind = identifierKind(i.type);
  }
  for (const d of record.idDocuments || []) {
    if (!d.kind) d.kind = identifierKind(d.type);
  }
  return record;
}

/*
 * Remove what annotateRecord added, for the write path.
 *
 * The annotations are derived — 112ms to rebuild across the whole snapshot at
 * load — and persisting them costs 7.3% on a 5MB blob that is committed to the
 * repository every single day. Recomputing is the cheaper side of that trade by
 * a wide margin, and it also means the stored snapshot stays exactly what the
 * authorities published, with this module's opinions layered on at read time
 * where they can be corrected by shipping code rather than by a rebuild.
 */
function stripAnnotations(record) {
  for (const a of record.attributes || []) { delete a.kind; delete a.date; }
  for (const i of record.identifiers || []) delete i.kind;
  for (const d of record.idDocuments || []) delete d.kind;
  return record;
}

module.exports = {
  attributeKind, identifierKind, annotateRecord, stripAnnotations, normalizeLabel,
